...
<script lang="ts">
	let { bookmarks = [], title, density, forceTitle = false, forceNoTitle = false }: Props = $props();

	const notEmptyTitle = $derived(title !== undefined && title.trim().length > 0);
</script>
...
<style>
	section {
		...
		&.large {
			background-color: color-mix(in srgb, var(--oub--foreground-color) 2%, transparent);
			border: solid 1px color-mix(in srgb, var(--oub--foreground-color) 6%, transparent);
			border-radius: 18px;
			padding: 15px;
		}
